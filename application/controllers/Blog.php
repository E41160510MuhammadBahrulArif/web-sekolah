 <?php
	...
	class Blog extends CI_Controller
	{
		...
		public function search_blog()
		{
			...
			$config['start'] 		= $this->uri->segment(4);
			$config['base_url'] 	= site_url('Blog/search_blog');
			$config['per_page'] 	= 10;
		...
		public function kategori($kategori = NULL)
		{
			...
			$config['base_url'] 	= site_url('Blog/kategori/' . $kategori);
			$config['per_page'] 	= 4;
			...
			if ($data['all_blog_kategori']->num_rows() > 0) {
				if ($this->M_Blog->get_blog_kategori($kategori)->num_rows() > 0) {
					...
					$this->load->view('layout/frontend/header', $data);
					$this->load->view('layout/frontend/navbar', $data);
					$this->load->view('frontend/blog/kategori', $data);
					$this->load->view('layout/frontend/footer');
				} else {
		...
		public function reply_comment()
		{
			...
			if ($this->form_validation->run() == TRUE) {
				...
				redirect('Blog/detail/' . $this->input->post('blog_slug'));
			} else {
